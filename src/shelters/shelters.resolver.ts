...
export class ShelterResolver {
  ...
  async residents(
    ...
    @Args('namePartial', { nullable: true }) namePartial?: string,
  ) {
    const { id } = shelter;
    let shelterResidents = residents.filter(
      resident => resident.shelterId == id,
    );
    if (namePartial) {
      shelterResidents = shelterResidents.filter(resident =>
        resident.name.toLowerCase().includes(namePartial.toLowerCase()),
      );
    }
